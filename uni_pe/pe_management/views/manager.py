    ...
    event_counts = (
        ...
        .annotate(
            ...
            created_by_manager_count=Count(
                "id", filter=Q(years_query, created_by_manager=True)
            ),
            number_count=Count(
                "id",
                filter=Q(
            ...
            total_created_by_manager_count=Count(
                "id", filter=Q(created_by_manager=True)
            ),
...
def events(request, structure_slug, structure=None):
    ...
    api_url = reverse(
        "pe_management:api_manager_events", kwargs={"structure_slug": structure_slug}
    )
    return render(
        request,
        template,
...
def new_event_choose_referent(request, structure_slug, structure=None):
    ...
    breadcrumbs = {
        ...
        reverse("pe_management:manager_management"): _("Management"),
        reverse(
            "pe_management:manager_events", kwargs={"structure_slug": structure_slug}
        ): structure.name,
        "#": _("New"),
    }
    ...
    if request.method == "POST":
        ...
        if referent_id.status_code != 200:
            ...
            return redirect(
                "pe_management:manager_new_event_choose_referent",
                structure_slug=structure_slug,
            )
        ...
        response = requests.get(
            "{}{}".format(API_ADDRESSBOOK_FULL, referent_id.json()),
            headers={"Authorization": "Token {}".format(settings.STORAGE_TOKEN)},
        )
        ...
        if not referent_data.get("Email"):
            messages.add_message(
                request, messages.ERROR, _("The person selected does not have an email")
            )
            return redirect(
                "pe_management:manager_new_event_choose_referent",
                structure_slug=structure_slug,
        ...
        return redirect(
            "pe_management:manager_new_event_basic_info", structure_slug=structure_slug
        )
        ...
        if form.is_valid():
            ...
            if not event.is_over():
                messages.add_message(
                    request,
                    messages.ERROR,
                    _("It is possible to add only ex-post events"),
                )
            else:
...
def event_people_delete(
    request, structure_slug, event_id, person_id, event=None, structure=None
):
...
def event_structures_delete(
    request, structure_slug, event_id, structure_id, event=None, structure=None
):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            log_action(
                ...
                obj=event,
                flag=CHANGE,
                msg="[Operatore di Ateneo] Dati di monitoraggio modificati"
                if instance
                else "[Operatore di Ateneo] Dati di monitoraggio inseriti",
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not event.created_by_manager:
                ...
                body = "{} {}.".format(
                    request.user, _("has changed the status of the event")
                )
            ...
            log_action(
                ...
                obj=event,
                flag=CHANGE,
                msg="[Operatore di Ateneo] Iniziativa riabilitata"
                if event.is_active
                else "[Operatore di Ateneo] Iniziativa disabilitata",
            )

            messages.add_message(
                request, messages.SUCCESS, _("Event status modified successfully")
            )
...
def export(request):
    ...
    if request.method == "POST":
        ...
        events = (
            ...
            .filter(structure__is_active=True, start__year=year)
