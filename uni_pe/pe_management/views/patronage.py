    ...
    event_counts = (
        ...
        .annotate(
            to_handle_count=Count(
                "id", filter=Q(patronage_operator_taken_date__isnull=True)
            ),
...
def events(request, structure_slug, structure=None):
    ...
    api_url = reverse(
        "pe_management:api_patronage_operator_events",
...
def event(request, structure_slug, event_id, structure=None):
    event = (
        PublicEngagementEvent.objects.prefetch_related("data")
        .filter(
            pk=event_id, structure__slug=structure_slug, data__patronage_requested=True
        )
        .first()
    )
    if not event:
        ...
        return redirect(
            "pe_management:patronage_operator_events", structure_slug=structure_slug
        )
...
def take_event(request, structure_slug, event_id, structure=None):
    ...
    if not event.can_be_handled_for_patronage():
        messages.add_message(request, messages.ERROR, _("Access denied"))
        return redirect(
            "pe_management:patronage_operator_event",
            structure_slug=structure_slug,
            event_id=event_id,
        )
    ...
    messages.add_message(request, messages.SUCCESS, _("Event taken successfully"))
...
def event_evaluation(request, structure_slug, event_id, structure=None):
    ...
    breadcrumbs = {
        ...
        reverse(
            "pe_management:patronage_operator_event",
            kwargs={"structure_slug": structure_slug, "event_id": event_id},
        ): "{}".format(event.title),
        "#": _("Evaluation"),
    }
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request, messages.SUCCESS, _("Patronage evaluation completed")
            )

            log_result = (
                "concesso" if form.cleaned_data["success"] == "True" else "negato"
            )
            msg = "[Patrocinio {}] Esito valutazione: {}".format(
            ...
            subject = '{} - "{}" - {}'.format(
                _("Public engagement"), event.title, _("Patronage evaluation completed")
            )
            body = "{} {}: {}.".format(
    ...
    return render(
        request,
        template,
        {"event": event, "form": form, "structure_slug": structure_slug},
    )
...
def event_reopen_evaluation(request, structure_slug, event_id, structure=None):
    ...
    # email
    subject = '{} - "{}" - {}'.format(
        _("Public engagement"), event.title, _("Patronage evaluation reopened")
    )
    body = "{} {} {}".format(
        request.user, _("has reopened patronage evaluation of the event"), "."
    )
    send_email_to_event_referents(event, subject, body)
