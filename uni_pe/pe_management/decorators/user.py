...
def has_report_editable(func_to_decorate):
    ...
    def new_func(*original_args, **original_kwargs):
        ...
        if event.has_report_editable()[0]:
            return func_to_decorate(*original_args, **original_kwargs)
        messages.add_message(request, messages.ERROR, event.has_report_editable()[1])
        return redirect(
            "pe_management:user_event", event_id=original_kwargs["event_id"]
        )
