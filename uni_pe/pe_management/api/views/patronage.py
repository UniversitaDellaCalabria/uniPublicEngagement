...
class PublicEngagementEventList(PublicEngagementEventList):
    permission_classes = [permissions.IsAuthenticated, IsStructurePatronageOperator]
    ...
    def get_queryset(self, **kwargs):
        ...
        if status == "to_handle":
            ...
            events = events.filter(
                patronage_granted=True, patronage_granted_date__isnull=False
            )
        elif status == "rejected":
            events = events.filter(
                patronage_granted=False, patronage_granted_date__isnull=False
            )
