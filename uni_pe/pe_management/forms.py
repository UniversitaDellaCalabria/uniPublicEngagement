...
class PublicEngagementReferentForm(forms.Form):
    event_owner = forms.BooleanField(label=_("I'm the event referent"))
...
class PublicEngagementEventForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if structure_slug:
            structures = OrganizationalStructure.objects.filter(slug=structure_slug)
    ...
    def clean(self):
        ...
        if start and start.year not in active_years:
            self.add_error(
                "start",
                _("It is not possible to enter dates for the year {}").format(
                    start.year
                ),
            )
        ...
        if (
            ...
            and end > timezone.now()
            and self.instance.id
            and hasattr(self.instance, "report")
        ):
            self.add_error(
                "end",
                _(
                    "Since the monitoring data is already present, the initiative must have already ended"
                ),
            )

        if (
            self.instance.id
            and hasattr(self.instance, "data")
            and start <= timezone.now()
        ):
            if (
                self.instance.data.patronage_requested
                or self.instance.data.promo_tool.exists()
                or self.instance.data.promo_channel.exists()
            ):
                self.add_error(
                    "start",
                    _(
                        "Data for the promotion of the initiative (promotion tools, promotion channels, etc...) and the request for patronage are not permitted if it has already started."
                    ),
                )
...
class PublicEngagementEventDisableEnableForm(forms.Form):
    notes = forms.CharField(
        label="Note", widget=forms.Textarea(attrs={"rows": 2}), required=True
    )
